...
console.log("Hello");
	...
	else
	{
		...
		for (let i = 0; i < res.length; i++) {
			...
			if(res[i].status == 0)
				var btn = `<button class="approveBtn" onClick="appproveReimbursement(` + res[i].ID + `, '` + userN + `')">Approve</button>   <button class="denyBtn" onClick="denyReimbursement(` + res[i].ID + `, '` + userN + `')">Deny</button>`;
			else
				var btn = `N/A`;
...
{
	let url = "/ReimbursementProject/LogoutServlet";
